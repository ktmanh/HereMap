...
//    addCategory(routingOptions, domRoute, 'routeMode');
...
slider.addEventListener('mouseup', function () {
    range = this.value;
    if (categoriesType && categoriesName) {
        via = null;
        removeRoute();
        removeMarker();
        removeMarker('via');
...
document.querySelector('#limitFinding').addEventListener('change', function () {
    var value = document.querySelector('#limitFinding').value;
    limit = value;
    if (categoriesType && categoriesName) {
        via = null;
        removeRoute();
...
var mainNav = {
    ...
    closeVia: function () {
        ...
        document.querySelector("#field-via-destination").dataset.position = null;
        document.querySelector("#field-via-destination").value = null;
        via = null;
        document.querySelector('.addVia').style.display = 'flex';
        removeMarker('via');
        if (JSON.parse(fieldDestination.getAttribute('data-position'))) {
    ...
    showDircection: function () {
        document.querySelector('.flex-top-form').style.display = 'none';
        document.querySelector('.flex-form').style.display = 'block';
        if (fieldLocation.dataset.position || fieldStarting.dataset.position) {
